...
- NivoSlider slides: main image is used as background (first image), second image is the product image with class 'nivo-slide-product' and should be visible/positioned over the background.
  - Always preserve and display product overlay images with class 'nivo-slide-product' by cloning them into the slider container and not hiding them; position them relative to the slider and ensure visibility (display/visibility/opacity) overrides.
